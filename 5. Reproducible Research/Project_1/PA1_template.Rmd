...
```{r}
Total_steps_filled <- tapply(df_filled$steps, df_filled$date, sum)
Total_steps_filled <- data.frame(date = names(Total_steps_filled), 
                                 total_steps = as.numeric(Total_steps_filled))
```
...
hist(Total_steps_filled$total_steps, 
     ...
     ylab = "Count", 
     main = "Total steps histogram with Imputed Data", 
     col = "darkblue", 
     breaks = 30, 
     xlim = c(0, 25000))
```
...
```{r}
t(data.frame(row.names = c("Median", "Mean"), 
             WithNA = as.numeric(summary(Total_steps$total_steps)[3:4]), 
             WithoutNA = as.numeric(summary(Total_steps_filled$total_steps)[3:4])))
```
