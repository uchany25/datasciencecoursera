...
```{r}
set.seed(42)
sample_data <- c(sample(en_US_blogs, length(en_US_blogs) * 0.3), 
                 sample(en_US_news, length(en_US_news) * 0.3),
                 sample(en_US_twitter, length(en_US_twitter) * 0.3))
...
candidates_3 <- c("decade", "weekend", "month", "morning")
...
```{r}
fragment_9 <- "I’m thankful my childhood was filled with imagination and bruises from playing"
